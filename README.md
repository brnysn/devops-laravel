...
### Provisioning
1. Update the server options in [`config.yml`](./config.yml) (PHP versions via `installs.php.versions` as comma-separated values like `8.3,8.4` where the first value becomes default `php`, database root password, optional services such as Reverb / Typesense, `deploy.keep_releases`, `installs.swapspace.max_size_in_mb`, and any additional node IPs in `servers`).
